        ...
        <div class="sales-container">
            ...
            <div class="kpi-grid">
                <div class="kpi-card">
                    ...
                    <p class="kpi-value">₡<?php echo e(number_format($dailySales, 0, ',', '.')); ?></p>
                    <div class="kpi-trend <?php echo e($dailySalesTrend >= 0 ? 'trend-up' : 'trend-down'); ?>">
                        <i class="fas fa-arrow-<?php echo e($dailySalesTrend >= 0 ? 'up' : 'down'); ?>"></i> <?php echo e(abs($dailySalesTrend)); ?>%
                    </div>
                </div>
                <div class="kpi-card">
                    <div class="kpi-header">
                    ...
                    <p class="kpi-value"><?php echo e($dailyTransactions); ?></p>
                    <div class="kpi-trend <?php echo e($dailyTransactionsTrend >= 0 ? 'trend-up' : 'trend-down'); ?>">
                        <i class="fas fa-arrow-<?php echo e($dailyTransactionsTrend >= 0 ? 'up' : 'down'); ?>"></i> <?php echo e(abs($dailyTransactionsTrend)); ?>%
                    </div>
                </div>
                <div class="kpi-card">
                    <div class="kpi-header">
                    ...
                    <p class="kpi-value"><?php echo e($refunds); ?></p>
                    <div class="kpi-trend <?php echo e($refundsTrend >= 0 ? 'trend-up' : 'trend-down'); ?>">
                        <i class="fas fa-arrow-<?php echo e($refundsTrend >= 0 ? 'up' : 'down'); ?>"></i> <?php echo e(abs($refundsTrend)); ?>
            ...
            <div class="filters-section">
                ...
                <form method="GET" action="<?php echo e(route('sales.index')); ?>" id="filters-form">
                    <div class="filters-grid">
                        <div class="filter-group">
                            ...
                            <select id="status" name="status">
                                ...
                                <option value="refunded" <?php echo e(request('status') == 'refunded' ? 'selected' : ''); ?>>Reembolsada</option>
                            </select>
                        </div>
                        <div class="filter-group">
                            <label for="date-range">Rango de Fechas</label>
                            <select id="date-range" name="date_range">
                                <option value="today" <?php echo e(request('date_range') == 'today' ? 'selected' : ''); ?>>Hoy</option>
                                <option value="week" <?php echo e(request('date_range') == 'week' ? 'selected' : ''); ?>>Esta semana</option>
                                <option value="month" <?php echo e(request('date_range') == 'month' ? 'selected' : ''); ?>>Este mes</option>
                        ...
                        <div class="filter-group">
                            ...
                            <select id="payment-method" name="payment_method">
                                ...
                                <option value="cash" <?php echo e(request('payment_method') == 'cash' ? 'selected' : ''); ?>>Efectivo</option>
                                <option value="sinpe" <?php echo e(request('payment_method') == 'sinpe' ? 'selected' : ''); ?>>SINPE Móvil</option>
                                <option value="transfer" <?php echo e(request('payment_method') == 'transfer' ? 'selected' : ''); ?>>Transferencia Bancaria</option>
                            </select>
                        </div>
                        <div class="filter-group">
                            <label for="search-sale">Buscar</label>
                            <input type="text" id="search-sale" name="search" placeholder="Buscar por cliente..." value="<?php echo e(request('search')); ?>">
                        </div>
                        <div class="filter-group filter-buttons">
                            <button type="submit" class="btn btn-primary filter-btn"><i class="fas fa-search"></i> Aplicar Filtros</button>
                            <a href="<?php echo e(route('sales.index')); ?>" class="btn btn-primary filter-btn"><i class="fas fa-times"></i> Limpiar</a>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <div class="sales-table-container">
                ...
                ?>
                        ...
                        <tr>
                            <td><strong><?php echo e($sale->invoice_number ?? '#' . $sale->sale_id); ?></strong></td>
                            <td><?php echo e($sale->customer->nombre ?? 'N/A'); ?> <?php echo e($sale->customer->apellido ?? ''); ?></td>
                            <td><?php echo e($sale->sale_date->format('d/m/Y H:i')); ?></td>
                            <td><span class="status-badge <?php echo e($sale->status); ?>"><?php echo e($statusLabels[$sale->status] ?? $sale->status); ?></span></td>
                            <td><?php echo e($paymentLabels[$sale->payment_method] ?? $sale->payment_method); ?></td>
                            <td><strong>₡<?php echo e(number_format($sale->total, 0, ',', '.')); ?></strong></td>
                            <td>
                                <div class="actions-container">
                                    <button class="action-btn view" onclick="viewSale('<?php echo e($sale->sale_id); ?>')" title="Ver detalles"><i class="fas fa-eye"></i></button>
                                    <?php if($sale->status === 'pending'): ?>
                                    <button class="action-btn success" onclick="completeSale('<?php echo e($sale->sale_id); ?>')" title="Completar venta"><i class="fas fa-check"></i></button>
                                    <button class="action-btn danger" onclick="cancelSale('<?php echo e($sale->sale_id); ?>')" title="Cancelar venta"><i class="fas fa-times"></i></button>
                                    <?php endif; ?>
                                    <?php if($sale->status === 'completed'): ?>
                                    <button class="action-btn warning" onclick="refundSale('<?php echo e($sale->sale_id); ?>')" title="Reembolsar"><i class="fas fa-undo"></i></button>
                                    <?php endif; ?>
                                    <button class="action-btn secondary" onclick="printSale('<?php echo e($sale->sale_id); ?>')" title="Imprimir"><i class="fas fa-print"></i></button>
                                </div>
                            </td>
                        </tr>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.pagination','data' => ['paginator' => $sales,'label' => 'de ventas']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['paginator' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($sales),'label' => 'de ventas']); ?>
        ...
        <div id="new-sale-modal" class="modal-overlay">
            <div class="modal-content modal-auto-size">
                ...
                <div class="modal-body">
                    <form id="new-sale-form" method="POST" action="<?php echo e(route('sales.store')); ?>">
                        <?php echo csrf_field(); ?>
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <select id="customer_id" name="customer_id" required>
                                    ...
                                    <?php $__currentLoopData = \App\Models\Usuario::where('rol', 'cliente')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($c->usuario_id); ?>"><?php echo e($c->nombre); ?> <?php echo e($c->apellido); ?></option>
                            ...
                            <div class="total-row">
                                ...
                                <span style="display: flex; align-items: center; gap: 8px;">
                                    <select id="iva_percentage" name="iva_percentage" style="width: 80px; padding: 6px 8px;">
                                        <?php for($p = 0; $p <= 13; $p++): ?>
                                        <option value="<?php echo e($p); ?>" <?php echo e($p == 0 ? 'selected' : ''); ?>><?php echo e($p); ?>%</option>
                                        <?php endfor; ?>
                                    </select>
...
<?php /**PATH /var/www/html/resources/views/sales/index.blade.php ENDPATH**/ ?>