...
{{-- Determine whether this is a Google-only account (no local password set) --}}
@php $isGoogleOnly = $client->google_id && empty($client->password); @endphp
...
<div class="container">
    <div class="profile-wrapper">
        ...
        <div id="profileAlert" class="alert alert-success hidden" role="alert">
            ...
            <button type="button" class="profile-alert-close"
                    onclick="closeProfileAlert()" aria-label="Cerrar">
        ...
        <div class="profile-grid">
            ...
            <div class="profile-card">
                ...
                <form id="formPerfil"
                      action="{{ route('clients.profile.update') }}"
                      method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="profile-fields">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="name" name="name"
                                   ...
                                   readonly required minlength="2" maxlength="60"
            ...
            <div class="profile-card">
                ...
                {{-- CTA shown for Google-only accounts that have no local password --}}
                @if($isGoogleOnly)
                <div id="googlePassCta" class="profile-google-cta">
                    ...
                    <p>
                        Actualmente solo puedes ingresar con tu cuenta de Google.<br>
                        Al definir una contraseña podrás iniciar sesión también
                        con tu correo y contraseña.
                    </p>
                    <button type="button" class="btn btn-primary btn-block"
                            onclick="showPasswordForm()">
                        <i class="fas fa-key"></i> Definir contraseña de acceso
                    </button>
                </div>
                @endif

                {{-- Form is hidden initially for Google-only accounts --}}
                <form id="formPassword"
                      action="{{ route('clients.profile.password') }}"
                      method="POST"
                      class="{{ $isGoogleOnly ? 'hidden' : '' }}">
                    @csrf
                    @method('PUT')

                    <div class="profile-fields">

                        {{-- Current password field shown only for local accounts --}}
                        @if(!$isGoogleOnly)
                        <div class="form-group profile-field-full">
                        ...
                        <div class="form-group">
                            ...
                            {{-- Strength indicator; activated via JS on input --}}
                            <div id="passStrength" class="profile-strength hidden">
                                ...
                                <span id="strengthLabel" class="profile-strength-label"></span>
                            </div>
                            @error('new_password')
                                <span class="profile-field-error">
                                    <i class="fas fa-exclamation-circle"></i> {{ $message }}
                    ...
                    <div class="profile-form-actions">
                        ...
                        {{-- Cancel button visible only for Google-only accounts --}}
                        @if($isGoogleOnly)
                        <button type="button" class="btn btn-secondary"
