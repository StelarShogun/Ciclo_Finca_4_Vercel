...
@push('styles')
    ...
    <style>
        ...
        /* ── Diff colours ── */
        .diff-positive { color: #b91c1c; font-weight: 600; }
        .diff-negative { color: #166534; font-weight: 600; }
        .diff-zero     { color: var(--color-text-secondary, #6b7280); }
...
@php
    ...
    $priceUpItems    = collect($items)->where('found', true)->filter(fn($i) => $i['suggested_sale_price'] !== null)->count();
@endphp
...
<div class="sales-container xml-review-container">
    ...
    <form
        method="POST"
        action="{{ route('admin.supplier-orders.xml-deviation.apply') }}"
        id="xml-apply-form"
    >
        @csrf
            ...
            <table class="xml-review-table">
                <thead>
                    <tr>
                        ...
                        <th>% Desvío</th>
                        {{-- New column --}}
                        <th title="Sólo aparece cuando el precio de compra sube. El valor sugerido mantiene el mismo % de margen actual. Puede editarlo o borrarlo para no modificar el precio de venta.">
                            <i class="fas fa-tag" style="color:#d97706;margin-right:.3rem;"></i>
                            Precio de venta
                            <i class="fas fa-info-circle" style="font-size:.75rem;color:#9ca3af;" title="Editable. Vacío = sin cambios."></i>
                        </th>
                        <th>Estado</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($items as $item)
                        @php
                            $rowClass  = ! $item['found'] ? 'row-not-found' : ($item['has_deviation'] ? 'row-deviation' : '');
                        ...
                        <tr class="{{ $rowClass }}" data-product-id="{{ $item['product_id'] ?? '' }}">
                                    ...
                                    <div class="sale-price-suggestion">
                                        ...
                                        <div style="display:flex;align-items:center;gap:.3rem;">
                                            <div class="sale-price-input-wrap" style="flex:1;">
                                                ...
                                                <input
                                                    type="number"
                                                    name="sale_prices[{{ $item['product_id'] }}]"
                            ...
                            <td>
                                @if (! $item['found'])
                                    <span class="badge-not-found"><i class="fas fa-times-circle"></i> No encontrado</span>
                                @elseif ($item['has_deviation'])
                                    <span class="badge-deviation"><i class="fas fa-exclamation-triangle"></i> Desvío detectado</span>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="10" style="text-align:center;padding:2rem;color:var(--color-text-secondary,#6b7280);">
        ...
        <div class="xml-reason-group">
            ...
            <textarea
                ...
                name="reason"
                maxlength="500"
                placeholder="Ej: Ajuste por alza generalizada de precios del proveedor XYZ en mayo 2025."
            ></textarea>
        </div>
...
(function () {
    ...
    // ── Quick-select helpers ─────────────────────────────────────────────────
    document.getElementById('btn-select-deviations').addEventListener('click', function () {
        checkboxes().forEach(cb => {
            cb.checked = cb.closest('tr').classList.contains('row-deviation');
        });
        updateCount();
    });
    ...
    // Highlight input when the admin changes the value away from the suggestion.
    ...
    // Clear button: empties the field so sale_price is NOT changed.
    ...
    document.getElementById('xml-apply-form').addEventListener('submit', function (e) {
        const selected = checkboxes().filter(c => c.checked).length;

        if (selected === 0) {
            return; // Controller handles gracefully.
        }

        // Count how many products will also have their sale_price changed.
        const saleChanges = [...document.querySelectorAll('.sale-price-input')]
            .filter(i => i.value !== '').length;

        let message = `¿Confirma la actualización del precio de compra de ${selected} producto(s)?`;

        if (saleChanges > 0) {
            message += `\n\nAdemás, se actualizará el precio de VENTA de ${saleChanges} producto(s).`;
        }
