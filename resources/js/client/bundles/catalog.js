...
export function initClientCatalogPage() {
  ...
  initCatalogSpotlightCarousel();
}

async function initCatalogSpotlightCarousel() {
    var root = document.querySelector('[data-catalog-spotlight-carousel]');
    if (!root) return;
    var swiperEl = root.querySelector('.swiper');
    if (!swiperEl) return;
    var [{ default: Swiper }, { Navigation, Autoplay, A11y }] = await Promise.all([
        import('swiper'),
    ...
    if (wrapperEl && slides.length > 1 && slides.length < minSlidesForLoop) {
        var originalSlides = Array.prototype.slice.call(slides);
        while (wrapperEl.querySelectorAll('.swiper-slide').length < minSlidesForLoop) {
            originalSlides.forEach(function (slide) { wrapperEl.appendChild(slide.cloneNode(true)); });
        }
        slides = swiperEl.querySelectorAll('.swiper-slide');
    }
    var delay = parseInt(root.getAttribute('data-autoplay-delay'), 10);
    ...
    try {
        new Swiper(swiperEl, {
            ...
            loopAdditionalSlides: 0,
            speed: 600,
            autoplay: { enabled: true, delay: delay, disableOnInteraction: false, pauseOnMouseEnter: false },
        ...
        if (console && console.error) console.error('Catalog spotlight carousel failed to init:', err);
    }
}
