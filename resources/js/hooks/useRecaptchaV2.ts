...
function loadRecaptchaScript(): Promise<void> {
  ...
  if (existing) {
    return new Promise((resolve) => {
      ...
      // If it's already loaded, 'load' won't fire again; resolve on next tick.
  ...
  return new Promise((resolve, reject) => {
    ...
    script.addEventListener('error', () => reject(new Error('Failed to load reCAPTCHA api.js')));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // If recaptcha fails to load, backend validation will return the error.
        setIsRendered(false);
      }
    })();
