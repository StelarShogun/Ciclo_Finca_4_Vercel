...
const sunVariants = {
  checked: { scale: 0, pathLength: 0 },
  unchecked: { scale: 1, pathLength: 1 },
};

const moonVariants = {
  checked: { scale: 1, pathLength: 1 },
  unchecked: { scale: 0, pathLength: 0 },
};
...
/**
 * Interruptor de tema animado (sol ↔ luna) con framer-motion.
 * Controlado: recibe `isDark` y `onToggle` para integrarse con el sistema
 * de temas existente (toggleTheme / data-theme).
 */
export function AnimatedThemeToggle({ isDark, onToggle, className, label }: AnimatedThemeToggleProps) {
  ...
  return (
    <button
      type="button"
      onClick={onToggle}
      className={`cf4-anim-theme-toggle${className ? ` ${className}` : ''}`}
      aria-label={accessibleLabel}
      aria-pressed={isDark}
      title={accessibleLabel}
    >
      <motion.span className="cf4-anim-theme-toggle__icon" animate={state} initial={false}>
