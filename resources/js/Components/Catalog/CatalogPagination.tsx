      ...
      >
        ...
        <div className="cf4-pagination-controls-row">
          ...
          <div className="cf4-pagination-jump">
            ...
            />
            <button
              className="go-button pagination-go-button"
              type="button"
              onClick={commitGo}
            >
              Ir
...
function PaginationLink({
  ...
  if (!href) {
    return (
      <span className="button" aria-disabled="true" tabIndex={-1}>
  ...
  return (
    <Link href={href} preserveScroll {...commonProps} aria-label={ariaLabel}>
      {variant === 'icon' ? <ChevronIcon direction={direction ?? 'next'} /> : label}
    </Link>
  );
}
